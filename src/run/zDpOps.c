...
zprint_diff_files(cJSON *zpJRoot, _i zSd) {
    ...
    if (zRepoDamaged == zpGlobRepo_[zRepoId]->repoState) { return -13; }

    if (zIsCommitDataType == zDataType) { zpTopVecWrap_= &(zpGlobRepo_[zRepoId]->commitVecWrap_); }
    else if (zIsDpDataType == zDataType) { zpTopVecWrap_ = &(zpGlobRepo_[zRepoId]->dpVecWrap_); }
    else { return -10; }
    ...
    if (NULL == zGet_OneCommitVecWrap_(zpTopVecWrap_, zCommitId)) {
        zMeta__ zMeta_ = {
            .repoId = zRepoId,
            .cacheId = zCacheId,
            .dataType = zDataType,
        ...
        /* 检测缓存是否正在生成过程中 */
        if (-7 == zGet_OneCommitVecWrap_(zpTopVecWrap_, zCommitId)->vecSiz) {
            pthread_rwlock_unlock(&(zpGlobRepo_[zRepoId]->rwLock));
    ...
    zSendVecWrap_.p_vec_ = zGet_OneCommitVecWrap_(zpTopVecWrap_, zCommitId)->p_vec_;
    zSplitCnt = (zGet_OneCommitVecWrap_(zpTopVecWrap_, zCommitId)->vecSiz - 1) / zSendUnitSiz  + 1;

    zNetUtils_.send_nosignal(zSd, "{\"ErrNo\":0,\"data\":", sizeof("{\"ErrNo\":0,\"data\":") - 1);  /* json 前缀 */
    for (_i zCnter = zSplitCnt; zCnter > 0; zCnter--) {  /* 正文 */
        if (1 == zCnter) {
            zSendVecWrap_.vecSiz = (zpTopVecWrap_->p_refData_[zCommitId].p_subVecWrap_->vecSiz - 1) % zSendUnitSiz + 1;
        } else {
...
zprint_diff_content(cJSON *zpJRoot, _i zSd) {
    ...
    if (NULL == zGet_OneFileVecWrap_(zpTopVecWrap_, zCommitId, zFileId)) {
        ...
        /* 检测缓存是否正在生成过程中 */
        if (-7 == zGet_OneFileVecWrap_(zpTopVecWrap_, zCommitId, zFileId)->vecSiz) {
            pthread_rwlock_unlock(&(zpGlobRepo_[zRepoId]->rwLock));
    ...
    zSendVecWrap_.vecSiz = 0;
    zSendVecWrap_.p_vec_ = zGet_OneFileVecWrap_(zpTopVecWrap_, zCommitId, zFileId)->p_vec_;
    zSplitCnt = (zGet_OneFileVecWrap_(zpTopVecWrap_, zCommitId, zFileId)->vecSiz - 1) / zSendUnitSiz  + 1;
    ...
    for (_i zCnter = zSplitCnt; zCnter > 0; zCnter--) {
        if (1 == zCnter) {
            zSendVecWrap_.vecSiz = (zGet_OneFileVecWrap_(zpTopVecWrap_, zCommitId, zFileId)->vecSiz - 1) % zSendUnitSiz + 1;
        } else {
            zSendVecWrap_.vecSiz = zSendUnitSiz;
        }
...
\
            ...
            "exec 777<&-;",\
\
            zpGlobRepo_[zRepoId]->p_repoPath + zGlobHomePathLen, zpGlobRepo_[zRepoId]->p_repoPath + zGlobHomePathLen,\
            zpGlobRepo_[zRepoId]->p_repoPath + zGlobHomePathLen, zpGlobRepo_[zRepoId]->p_repoPath + zGlobHomePathLen,\
            zpGlobRepo_[zRepoId]->p_repoPath + zGlobHomePathLen, zpGlobRepo_[zRepoId]->p_repoPath + zGlobHomePathLen,\
...
zupdate_ip_db_all(_i zRepoId, char *zIpList, _ui zIpCnt, char *zpCommonBuf, zRegRes__ **zppRegRes_Out) {
    ...
    zRegRes__ *zpRegRes_, zRegRes_[1] = {{.alloc_fn = zNativeOps_.alloc, .repoId = zRepoId}};  // 使用项目内存池
    zpRegRes_ = zRegRes_;  /* avoid compile warning... */

    zPosixReg_.init(zRegInit_ , "[^ ]+");  /* IP 之间必须以空格分割 */
    zPosixReg_.match(zRegRes_, zRegInit_, zIpList);
    zPosixReg_.free_meta(zRegInit_);
    *zppRegRes_Out = zpRegRes_;

    if (zIpCnt != zRegRes_->cnt) { return -28; }
    ...
    for (_ui zCnter = 0; zCnter < zRegRes_->cnt; zCnter++) {
        /* 检测是否存在重复IP */
        if (0 != zpGlobRepo_[zRepoId]->p_dpResList_[zCnter].clientAddr[0] || 0 != zpGlobRepo_[zRepoId]->p_dpResList_[zCnter].clientAddr[1]) {
        ...
        zpGlobRepo_[zRepoId]->p_dpCcur_[zCnter].p_hostIpStrAddr = zRegRes_->p_rets[zCnter];
        ...
        zConvert_IpStr_To_Num(zRegRes_->p_rets[zCnter], zpGlobRepo_[zRepoId]->p_dpResList_[zCnter].clientAddr, zErrNo);
        if (0 != zErrNo) {
            free(zpGlobRepo_[zRepoId]->p_dpResList_);
...
zExistMark:;
    ...
    if ((-1 == zpGlobRepo_[zRepoId]->resType[0])
        ...
        if ((10 < zpGlobRepo_[zRepoId]->totalHost) && ( zFailedHostCnt < zpGlobRepo_[zRepoId]->totalHost / 10)) { return 0; }
        else { return -23; }
    }
...
zdeploy(_i zSd, _i zRepoId, _i zCacheId, _i zCommitId, _i zDataType, char *zpIpList, _i zIpCnt, char **zppCommonBuf, zRegRes__ **zppIpAddrRegRes_OUT) {
    ...
    for (zCnter = 0; zCnter < zpGlobRepo_[zRepoId]->totalHost; zCnter++) {
        zOffSet += sprintf(zppCommonBuf[0] + zOffSet, "($1,$2,$3,'%s'),", zpGlobRepo_[zRepoId]->p_dpCcur_[zCnter].p_hostIpStrAddr);
    }
    ...
    if (NULL == (zpPgResHd_ = zPgSQL_.exec_with_param(zpGlobRepo_[zRepoId]->p_pgConnHd_, zppCommonBuf[0], 3, zppParam, zFalse))) {
    ...
    /* 最长 10 分钟 */
    if (600 < zpGlobRepo_[zRepoId]->dpTimeWaitLimit) { zpGlobRepo_[zRepoId]->dpTimeWaitLimit = 600; }
    ...
    if (90 < zpGlobRepo_[zRepoId]->dpTimeWaitLimit) {
        _i zSendLen = sprintf(zppCommonBuf[0], "{\"ErrNo\":-14,\"content\":\"本次布署时间最长可达 %zd 秒，请稍后查看布署结果\"}", zpGlobRepo_[zRepoId]->dpTimeWaitLimit);
    ...
    if (-1 == zpGlobRepo_[zRepoId]->resType[1]) { goto zErrMark; }

    if (zpGlobRepo_[zRepoId]->totalHost == zpGlobRepo_[zRepoId]->dpReplyCnt) {
        zErrNo = 0;
    } else if ( ((10 <= zpGlobRepo_[zRepoId]->totalHost) && ((zpGlobRepo_[zRepoId]->totalHost * 9 / 10) <= zpGlobRepo_[zRepoId]->dpReplyCnt))) {
        /*
...
zErrMark:
    ...
    if (0 != strcmp(zGet_OneCommitSig(zpTopVecWrap_, zCommitId), zpGlobRepo_[zRepoId]->lastDpSig)) {
        ...
        sprintf(zppCommonBuf[0], "cd %s; git branch -f `git log CURRENT -1 --format=%%H`; git branch -f CURRENT", zpGlobRepo_[zRepoId]->p_repoPath);
        if (0 != WEXITSTATUS( system(zppCommonBuf[0])) ) {
            zPrint_Err(0, NULL, "\"CURRENT\" branch refresh failed");
        }

        /* 若已确认全部成功，重置内存池状态 */
        if (0 == zErrNo) { zReset_Mem_Pool_State(zRepoId); }
        ...
        while(1) {
            ...
            for (_l zTimeCnter = 0; zpGlobRepo_[zRepoId]->dpTimeWaitLimit > zTimeCnter; zTimeCnter++) {
                if ((0 != zpGlobRepo_[zRepoId]->whoGetWrLock)  /* 检测是否有新的布署请求或已全部布署成功 */
                        || ( (zpGlobRepo_[zRepoId]->totalHost == zpGlobRepo_[zRepoId]->dpReplyCnt) && (-1 != zpGlobRepo_[zRepoId]->resType[1]))) {
