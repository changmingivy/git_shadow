...
zbatch_deploy(cJSON *zpJRoot, _i zSd) {
    ...
    /* 每次尝试将 ____shadowXXXXXXXX 分支删除，避免该分支体积过大，不必关心执行结果 */
    zLibGit_.branch_del(zRun_.p_repoVec[zRepoId]->p_gitRepoHandler, "____shadowXXXXXXXX");

    /*
     * 提交到 ____shadowXXXXXXXX 分支，确保每次 push 都能触发 post-update 勾子
     */
    if (0 != zLibGit_.add_and_commit(zRun_.p_repoVec[zRepoId]->p_gitRepoHandler, "____shadowXXXXXXXX", "HookAwaker", "_")) {
        zErrNo = -15;
        goto zCleanMark;
    }
        ...
        if (0 == zRun_.p_repoVec[zRepoId]->p_dpResList_[i].errState) {  /* 无错 */
            ...
            if (0 != zConvert_IpNum_To_Str(zRun_.p_repoVec[zRepoId]->p_dpResList_[i].clientAddr, zIpStrBuf)) {
                ...
                for (j = 0; j < zErrClassNum; j++) {
                    if (zCheck_Bit(zRun_.p_repoVec[zRepoId]->p_dpResList_[i].errState, j + 1)) {
