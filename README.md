# 一、概述    
&emsp;&emsp;**git_shadow** 是一套纯 C 语言实现的、基于 libgit2 的高性能布署系统后端，专注于最终线上产品的分发环节，支持无限量大规模布署场景；同时具备监控数据收集、分析功能；用于支撑本人另一个项目 [svdp](https://github.com/kt10/svdp.git) 的布署功能。
#### 1. 系统环境需求    
...
|**content**|存放错误信息|
>4. **注意事项**       
    - 服务端项目路径、数据库中的项目元信息均会被删除；目标机端的项目路径不会被删除。       
## B、查询类接口    
### [opsID 9] 打印版本号列表
>1. **功能描述**       
